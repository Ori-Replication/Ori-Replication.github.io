...
本期视频是一个教学视频，所以推荐大家拿出电脑跟着我做。视频主要面向需要进行深度学习训练的同学们，剩下的我也不会。有什么问题也欢迎大家加入炼丹群里面和群友提问。炼丹群就不贴在这了，找你的校内同学要或者翻翻邮件，找那次慧炼丹心杯的邮件应该能找到。
## 集群申请
有同学说：主播主播，学校HPC确实很强，但我没有账号怎么办。有办法的兄弟。
### 方法1
第一种最不吃操作的方式是向学校的管理信院AI集群的闻天明老师发一封诚恳的邮件，说明你需要用学校集群干什么，以及使用的时间等。礼貌、合理地交涉还是有很大机会的。闻天明老师的邮箱大家可以在[上海科技大学协同管理系统 ](https://oa.shanghaitech.edu.cn/wui/main.jsp?templateId=1) 里面找到
### 方法2
第二种不那么吃操作的方法是参加一些需要用学校集群的课程，会为你们提供学校集群账号。
### 方法3
又有同学要说了，主播主播，你的方法确实很简单，但是信院AI集群卡太难抢了，normal队列卡太破了。有没有更强势的方法。有的兄弟有的。你需要找你的学业导师帮帮忙，让他帮你填写一个申请就可以拿到一个P40集群的账号。申请的流程链接会放在文稿版中。[账号申请](https://it.shanghaitech.edu.cn/8850/list.htm)
...
```
### 方法4
又有同学说，主播主播，你的方法确实很强，但P40还是太弱小了，有没有更加牛逼的卡？有的有的兄弟，你如果能够进入信院的某些课题组，就有机会使用最神秘的A800集群。但是比较吃操作，大家各凭本事。
...
![[attachments/Pasted image 20250218211832.png]]
...
点击你vscode左下角的这个东西
![[attachments/Pasted image 20250218211921.png]]
![[attachments/Pasted image 20250218212557.png]]

![[attachments/Pasted image 20250218213519.png]]

选择用户名下的config文件
![[attachments/Pasted image 20250218214724.png]]

接着，在这里面写下这样一条配置：
```
Host hongmt2022-sist-ai(任意，你想取的)
  HostName 10.15.xx.xxx(服务器IP，四个由 . 分割的0-255之间的数字)
  Port xxxxx(服务器端口，0-25565)
  User (管理员给你的用户名)
```
...
ok我这里大概等了五分钟啊，它加载好了。我们需要再输入一次密码，然后再等一会，直到左下角的下载vscode服务器走完。
这里等了大概半分钟，它下载好了。这时候上方会显示出你的工作区路径，点一下确定。它会让我们再输一次密码，你就再输一次。现在我们终于是进入了我们的服务器。
...
![[attachments/Pasted image 20250221232946.png]]
...
![[attachments/Pasted image 20250223120055.png]]
...
![[attachments/Pasted image 20250223121136.png]]
...
## 申请资源与任务执行
这里我估计大家的conda还没下完，就先介绍下怎么申请计算资源。
...
![[attachments/Pasted image 20250223002141.png]]
试着用指定节点的方式申请吧。
...
3. partition代表你的队列。比如我这个账号只有normal的权限。具体能够申请到哪些节点，可以通过集群手册去看。比如我是normal队列，能够使用的GPU是 ai_gpu14~25，sist_gpu36~66，最多能够申请的GPU是12块。但这里测试了一下，能够使用的GPU变成了 ai_gpu 18~25, sist_gpu 36~70.这一点集群手册没有更新。因为 14-17这几个节点换了最新的L20显卡，没有开放给normal用户用。![[attachments/Pasted image 20250222200644.png]]
...
有些集群中，仅使用 `exit` 无法释放节点。需要手动进行 `sancel` 。信院AI集群的资源会无占用在半小时后自动释放。但在其余管理比较宽松的集群中需要注意手动进行 `scancel`，否则资源不会自动释放，会导致资源浪费。
...
![[attachments/Pasted image 20250223130335.png]]
...
## 结束语
本期的教程大概就到这里了。信院AI集群限制还是比较多的，并且normal用户能拿到的卡也一般。因此大家如果自己电脑里面有显卡，平时也可以在自己电脑里面炼丹。当然挂在集群上炼丹也有好处，它不会占用你电脑的资源。如果在自己电脑上面炼，你可能就干不了别的事情了。有钱的同学也可以选择去各种平台租卡炼丹，价格一般在 3-5 元/卡/小时。
