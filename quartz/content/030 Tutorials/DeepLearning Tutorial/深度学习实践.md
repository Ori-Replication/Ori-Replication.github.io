...
#### len 方法
接着，是 len 方法。需要正确地返回样本的长度。这个方法必须实现，如果你使用dataset
```python
    def __len__(self):
        """返回数据集总样本数"""
...
- **参数更新公式**：
  $$
\begin{align*}
  m_t = \beta_1 m_{t-1} + (1-\beta_1) \nabla_\theta J(\theta) \quad \text{（一阶矩，均值）} \\
  v_t = \beta_2 v_{t-1} + (1-\beta_2) (\nabla_\theta J(\theta))^2 \quad \text{（二阶矩，未中心化方差）} \\
  \hat{m}_t = \frac{m_t}{1-\beta_1^t}, \quad \hat{v}_t = \frac{v_t}{1-\beta_2^t} \quad \text{（偏差校正）} \\
  \theta_{t+1} = \theta_t - \frac{\eta}{\sqrt{\hat{v}_t} + \epsilon} \hat{m}_t
\end{align*}
$$
  - $\beta_1=0.9$（动量衰减率），$\beta_2=0.999$（方差衰减率）
  - $\epsilon=1e-8$：防止除零错误
...
| 优化器     | 收敛速度 | 内存消耗 | 超参数敏感度 | 适用场景       |
| ------- | ---- | ---- | ------ | ---------- |
| SGD     | 慢    | 低    | 高      | 大数据简单任务    |
| Adam    | 快    | 中    | 低      | 复杂模型快速开发   |
| AdamW   | 快    | 高    | 中      | 大规模复杂模型训练  |
为什么 Adam 会消耗更多的显存？
1. **状态变量存储** ：
    - **Adam** 维护两个状态变量：
        - ( m_t )：梯度的一阶矩（均值）。
