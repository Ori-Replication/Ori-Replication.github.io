---
title: A Glimpse into the Essence of Tumors and Cancer
publish: true
tags:
---
...
However, today's lecture provided a new understanding of tumor development. Professor Xu Ying posits that the onset of cancer is due to "chronic inflammation," with gene mutations being the result of selection, not the cause. If certain factors (e.g., radiation, infection, tobacco) cause immune cells to continuously react to a specific area, chronic inflammation can occur. In the inflamed area, immune cells release hydrogen peroxide ($H_{2}O_{2}$) and superoxide ($O_{2}^{-}$) in an attempt to kill invaders. This chronic inflammation damages local tissues, triggering repair processes. During tissue repair, macrophages release growth factors and cytokines, leading to excessive cell proliferation and increased selective pressure due to oxidative stress.

### First Pressure: Oxidative Stress
Oxidative stress occurs when the balance between oxidation and reduction in cells is disrupted, leading to an accumulation of reactive oxygen species (ROS) and reactive nitrogen species (RNS), which cause cell damage and dysfunction.
ROS includes:
...
Hydroxyl radicals can be produced through the Fenton reaction:
$$\text{Fe}^{2+} + \text{H}_2\text{O}_2 \rightarrow \text{Fe}^{3+} + \cdot\text{OH} + \text{OH}^-$$
This reaction produces hydroxide ions, making the environment alkaline. This will be discussed later.
